-- ============================================================
-- Silver: Display & Video 360 cleaned staging model
-- Source: marketing_bronze.dv360_raw
-- Grain: date × campaign × insertion_order × line_item
-- ============================================================
{{
  config(
    materialized      = 'incremental',
    unique_key        = ['report_date','campaign_id','insertion_order_id','line_item_id'],
    incremental_strategy = 'merge',
    partition_by      = {'field': 'report_date', 'data_type': 'date'},
    cluster_by        = ['campaign_id','line_item_type'],
    tags              = ['silver','dv360']
  )
}}

WITH

-- Step 1: Deduplicate
deduped AS (
    SELECT *,
           ROW_NUMBER() OVER (
               PARTITION BY date, campaign_id, insertion_order_id, line_item_id
               ORDER BY _processed_at DESC
           ) AS rn
    FROM {{ source('bronze', 'dv360_raw') }}
    {% if is_incremental() %}
    WHERE _pipeline_date >= DATE_SUB(CURRENT_DATE(), INTERVAL {{ var('lookback_days') }} DAY)
    {% endif %}
),

raw AS (SELECT * FROM deduped WHERE rn = 1),

-- Step 2: Parse date
parsed_date AS (
    SELECT
        *,
        CASE
            WHEN REGEXP_CONTAINS(date, r'^\d{4}-\d{2}-\d{2}$')       THEN PARSE_DATE('%Y-%m-%d', date)
            WHEN REGEXP_CONTAINS(date, r'^\d{2}/\d{2}/\d{4}$')       THEN PARSE_DATE('%d/%m/%Y', date)
            WHEN REGEXP_CONTAINS(date, r'^\d{2}-\d{2}-\d{4}$')       THEN PARSE_DATE('%d-%m-%Y', date)
            WHEN REGEXP_CONTAINS(date, r'^\d{8}$')                    THEN PARSE_DATE('%Y%m%d', date)
            WHEN REGEXP_CONTAINS(date, r'^[A-Za-z]+ \d{2}, \d{4}$')  THEN PARSE_DATE('%B %d, %Y', date)
            ELSE NULL
        END AS report_date
    FROM raw
),

-- Step 3: Clean and cast
cleaned AS (
    SELECT
        report_date,

        -- Identifiers
        NULLIF(TRIM(partner_id), '')                                        AS partner_id,
        NULLIF(TRIM(advertiser_id), '')                                     AS advertiser_id,
        NULLIF(TRIM(campaign_id), '')                                       AS campaign_id,
        NULLIF(TRIM(insertion_order_id), '')                                AS insertion_order_id,
        NULLIF(TRIM(insertion_order_name), '')                              AS insertion_order_name,
        NULLIF(TRIM(line_item_id), '')                                      AS line_item_id,
        NULLIF(TRIM(line_item_name), '')                                    AS line_item_name,
        UPPER(TRIM(NULLIF(line_item_type, '')))                             AS line_item_type,
        UPPER(TRIM(NULLIF(targeting_type, '')))                             AS targeting_type,
        NULLIF(TRIM(audience_segment), '')                                  AS audience_segment,
        NULLIF(TRIM(exchange), '')                                          AS exchange,

        -- Metrics: clamp negatives to NULL or 0
        CASE
            WHEN SAFE_CAST(impressions AS INT64) < 0 THEN NULL
            ELSE SAFE_CAST(impressions AS INT64)
        END AS impressions,

        CASE
            WHEN SAFE_CAST(clicks AS INT64) < 0 THEN NULL
            ELSE SAFE_CAST(clicks AS INT64)
        END AS clicks,

        GREATEST(SAFE_CAST(revenue_usd AS FLOAT64), 0)                     AS revenue_usd,
        GREATEST(SAFE_CAST(total_conversions AS FLOAT64), 0)               AS total_conversions,

        -- View rate: cap at [0, 1]
        CASE
            WHEN SAFE_CAST(view_rate AS FLOAT64) < 0 THEN 0
            WHEN SAFE_CAST(view_rate AS FLOAT64) > 1 THEN NULL  -- implausible
            ELSE SAFE_CAST(view_rate AS FLOAT64)
        END AS view_rate,

        -- CPM: negative means bad data
        CASE
            WHEN SAFE_CAST(average_cpm AS FLOAT64) < 0 THEN NULL
            ELSE SAFE_CAST(average_cpm AS FLOAT64)
        END AS average_cpm_usd,

        -- Derived CTR
        SAFE_DIVIDE(
            GREATEST(SAFE_CAST(clicks AS FLOAT64), 0),
            GREATEST(SAFE_CAST(impressions AS FLOAT64), 1)
        )                                                                   AS ctr,

        -- Pipeline metadata
        _source,
        _pipeline_date,
        _processed_at,
        _validation_pass

    FROM parsed_date
    WHERE report_date IS NOT NULL
),

valid AS (
    SELECT * FROM cleaned
    WHERE campaign_id IS NOT NULL
      AND line_item_id IS NOT NULL
)

SELECT * FROM valid
